...
abstract contract ERC20Token is ERC20Interface {
    ...
    uint256 public totalSupplyNumberOfTokens;  // Total number  of tokens
    ...
    constructor(
        ...
        balances[msg.sender] = _initialAmount;  // The creator owns all inital tokens
    ...
    // Transfer tokens from one specific address to another specific address
    function transferFrom(address _from, address _to, uint256 _value) override public returns (bool success) {
        uint256 allowanceVariable = allowed[_from][msg.sender];
        require(balances[_from] >= _value && allowanceVariable >= _value, "Insufficient allowed funds for transfer source...");
        balances[_from] -= _value;
        balances[_to] += _value;
        if (allowanceVariable < MAX_UINT256) {
    ...
    // Return the current balance (in tokens) of a specific address
